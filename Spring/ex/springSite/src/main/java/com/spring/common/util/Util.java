...
public class Util {
	...
	/***
	 * verify() 메소드는 입력값에 대한 유효성 체크 메소드
	 * @param(유효성 패턴, 패턴을 비교할 입력값)
	 * 참고 : .: 임의의 한문자 /? : 문자가 존재하지 않거나 하나의 문자 /
	 * * : 문자가 존재하지 않거나 하나 이상 반복 / + : 문자가 한번 이상 반복
	 ...
	 * 128비트의 수이다. 표준형식에서 UUID 는 32개의 16진수로 표현되며 총 36개(32개
	 * 문자와 4개의 하이픈) 문자로 된 8-4-4-4-12라는 5개의 그룹을 하이픈으로 구분한다.
	 * 이를테면 다음과 같다. 853b5ba5-1232-1242-334v-aewsf23341eaef
