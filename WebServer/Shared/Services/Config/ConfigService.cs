...
namespace Shared.Services.Config
{
    public class ConfigService
    {
        ...
        static public void InitPath()
        {
            ...
            int index = Directory.GetCurrentDirectory().LastIndexOf("WebServer");
            if (0 < index)
            {
                _basePathDir = Directory.GetCurrentDirectory().Remove(Directory.GetCurrentDirectory().LastIndexOf("WebServer"));
                _basePathDir += @"Config/";
            }
        }
